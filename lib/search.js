...
module.exports = new class LibSearch {
    ...
    optimizeSubs(response, input) {
        ...
        return Promise.all(response.map(sub => {
            ...
            const tmp = {
                url: input.gzip ? sub.SubDownloadLink : sub.SubDownloadLink.replace('.gz', '').replace('/src-api', '').replace(/\/vrf-(.*)\/file/, '/file'),
                referrer: sub.SubtitlesLink.replace('http:', 'https:').replace(/\/sid-.*\//, '/'),
                langcode: sub.ISO639,
